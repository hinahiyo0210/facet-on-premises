...
class ContractorController {
    ...
    public function add($tenant):array {
        ...
        if ($res) {
          ...
          $log->info($_SERVER["PHP_AUTH_USER"],"contractor登録処理完了:{\"contractor_id\":".$next_auto_num.",\"api_token\":\"".$api_token."\"}", date('Y-m-d H:i:s'));
        } else {
        ...
        $pdo = $db->pdo();
        $sth = $pdo->prepare($sql);
        $sth->bindValue(":contractor_id", $next_auto_num);
        $sth->bindValue(":ws_api_url", $ws_api_url_list[$tenant]);
        $sth->bindValue(":s3_path_prefix", substr('00000'.$next_auto_num, -6).'/picture/XXXXXXXXXX');
        $res = $sth->execute();
        $id = $pdo->lastInsertId();
        if ($res) {
          array_push($result_arr, true);
          $log->info($_SERVER["PHP_AUTH_USER"],"device登録処理完了:{\"contractor_id\":".$next_auto_num.",\"device_id\":".$id."}", date('Y-m-d H:i:s'));
        } else {
          array_push($result_arr, false);
        ...
        if ($res) {
            array_push($result_arr, true);
            $log->info($_SERVER["PHP_AUTH_USER"],"user登録処理完了:{\"contractor_id\":".$next_auto_num.",\"user_id\":".$id.",\"login_id\":\"".$post["login_id"]."\"}", date('Y-m-d H:i:s'));
        } else {
            array_push($result_arr, false);
        ...
        if(in_array(false, $result_arr)){
            ...
            $log->info($_SERVER["PHP_AUTH_USER"],"登録処理完了:{\"contractor_id\":".$next_auto_num."}", date('Y-m-d H:i:s'));

            $this->code = 201;
            header("Location: ".$this->url.$id);
            return [
              "msg" => "登録処理が完了しました。",
              "contractor_id" => $next_auto_num,
              "api_token" => $api_token
            ];
        }
