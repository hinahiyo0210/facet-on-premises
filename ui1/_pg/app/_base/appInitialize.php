...
spl_autoload_register(function($class) {
	...
	if (endsWith($class, "Service")) {
		...
		if (file_exists(API_SERVICE_DIR.'/'.$class.'.php')) {
			require_once(API_SERVICE_DIR.'/'.$class.'.php');
			return;
		}
		foreach (glob(API_SERVICE_DIR.'/*/'.$class.'.php') as $filename) {
			require_once($filename);
			return;
		}
	}
	...
	if (endsWith($class, "Exception")) {
		if (file_exists(API_SERVICE_DIR.'/exception/'.$class.'.php')) {
			require_once(API_SERVICE_DIR.'/exception/'.$class.'.php');
			return;
		}
	}
