 ...
 */
function sendRedirect($url, $addParam = "", $anc = "") {
	
	$url = addUrlParam($url, $addParam);
	
	header("Location: {$url}{$anc}");
	die;
}


function sendCompleteRedirect($url, $msg, $anc = "") {
	$key = Session::setMessage($msg);
	sendLocalRedirect($url, "msg=$key", $anc);
}
 ...
 * @param string $addParam URLに追加するgetパラメータ
 */
function sendLocalRedirect($url, $addParam = "", $anc = "") {
	
	if (exists($url, "://") || exists(urldecode($url), "://")) {
		response400("不正なパラメータです。");
	}
	
	sendRedirect($url, $addParam, $anc);
}
...
// 配列からURLパラメータを作成する。
function createQueryString($form, $excludes = []) {

	$params = [];
	foreach ($form as $k=>$v) {
		if (exists($excludes, $k)) continue;
		if (is_array($v)) {
			foreach ($v as $_v) $params[] = urlencode($k."[]")."=".urlencode($_v);
		} else {
			$params[] = (isset($k) && isset($v)) ? urlencode($k)."=".urlencode($v) : "=";
		}
	}
...
//url too long 対応
function createQueryStringExcludePulldown($form, $excludes = []) {
	
	if ($key = $form["_form_session_key"]){
		$excludes = array_merge($excludes, array_keys(Session::getData($key, false, 'dsFormSession')));
	}
	return createQueryString($form,$excludes);
}